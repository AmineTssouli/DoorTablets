  ...
  return (
   <div style={{display:'flex',justifyContent:'center',marginTop:50,alignItems:'center'}}>
       <>
           ...
           <label style={{marginRight:20,marginLeft:20}} > Building:</label>
            <select tyle={{width:230}} value={building} onChange={handleOnChange} id="buildings">
            <option  defaultValue value={0} >Choose a building</option>
            
            <ListBuildings />
          
            </select> 
            <label style={{marginRight:20,marginLeft:20}} > Room:</label>
            <select style={{width:230}} value={room} onChange={(event)=> {setRoom(event.target.value)}} id="rooms">
            <option defaultValue value={0} >Choose a room</option>
            {
              building && <ListRooms />
            }
            </select>
